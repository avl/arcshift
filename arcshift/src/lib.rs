...
fn do_janitor_task<T: ?Sized, M: IMetadata>(
    ...
    loop {
        ...
        if cur.strong_count.load(Ordering::SeqCst) > 0 {//atomic janitor check strong_count
    ...
    pub fn try_get_mut(&mut self) -> Option<&mut T> {
        ...
        with_holder!(self.item, T, |item_ptr: *const ItemHolder<T, _>| {
            ...
            if get_weak_count(weak_count) == 1
                ...
                && item.strong_count.load(Ordering::SeqCst) == 1//atomic try_get_mut check strong count
            {
